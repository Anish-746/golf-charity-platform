...
export async function POST(
  ...
  if (typedDraw.status !== 'simulated') {
    return NextResponse.json(
      { error: 'Draw must be simulated before publishing. Run simulation first.' },
      { status: 400 }
    )
  }

  if (!typedDraw.winning_numbers || typedDraw.winning_numbers.length !== 5) {
    return NextResponse.json({ error: 'No winning numbers set' }, { status: 400 })
  }
  ...
  const winnerPrizes = calculateWinnerPrizes(results, {
    jackpotPool: typedDraw.jackpot_amount ?? 0,
    fourMatchPool: typedDraw.four_match_pool ?? 0,
    threeMatchPool: typedDraw.three_match_pool ?? 0,
    totalPool:
      (typedDraw.jackpot_amount ?? 0) +
      (typedDraw.four_match_pool ?? 0) +
      (typedDraw.three_match_pool ?? 0),
  }) as WinnerPrize[]
